...
# ==================================================
# REWRITTEN DATA INTELLIGENCE ENGINE
# ==================================================
class DataBrain:
    def __init__(self):
        self.df = None
    ...
    def initialize(self):
        try:
            ...
            if not os.path.exists("nova_ozone_model.pkl") or not os.path.exists("nova_stats.json"):
                print("Error: Model files missing.")
                return False

            # 2. Load Visual Data (Needed for graphs and maps)
            self.df = pd.read_csv("Receptor_western_NAmerica_ozone_obs_1994_2021_from900to300.csv")
            
            # 3. Load Pre-calculated Stats
            with open("nova_stats.json", "r") as f:
                self.stats = json.load(f)
            self.accuracy = self.stats.get('accuracy', 0.0)
            
            # 4. Load Pre-trained AI Model
            self.model = joblib.load("nova_ozone_model.pkl")
...
class OzoneEnterpriseApp:
    def __init__(self, root):
        ...
        if not brain.initialize():
            messagebox.showerror(
                "System Failure", 
                "Could not load AI model or dataset.\n\nPlease run 'train_model.py' first to generate 'nova_ozone_model.pkl'."
            )
            root.destroy()
            return
    ...
    def render_graph(self, mode):
        ...
        if mode == "trend":
            ...
            sample = brain.df.sample(min(500, len(brain.df)))
    ...
    def run_simulation(self, parent):
        ...
        map_frame = tk.Frame(parent, height=500, bg="black")
